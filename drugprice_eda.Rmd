...
partd %>%
  group_by(`Brand Name`, Year) %>%
  summarise(avgspendperclaimbrand = sum(`Total Spending`)/sum(`Total Claims`)) %>%
  right_join(partd, by = c("Brand Name" = "Brand Name", "Year" = "Year")) %>%
  mutate(avgspendperclaimperdiff = ifelse((avgspendperclaimbrand/`Average Spending Per Claim`) > 1, "Above Average", ifelse((avgspendperclaimbrand/`Average Spending Per Claim`) < 1, "Below Average", "At Average"))) %>%
  group_by(Manufacturer, Year, avgspendperclaimperdiff) %>%
  tally() %>%
  spread(avgspendperclaimperdiff, n) %>%
  arrange(Year, desc(`Above Average`))
```
