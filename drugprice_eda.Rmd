...
partd %>%
  ...
  labs(x = "Inflation Adj. Spend", y = "Total Claims", title = "Bivariate relationship between Total Spend and Total Claims") +
  theme_minimal()
...
There are several interesting things to note of the bivariate relationship of spend and claims:

1. There seems to be a greater density of brands residing closer to either the x or y axis in recent years.
2. There are several extreme observations that have either increased dramatically in the number of claims per drug or in the spend per drug, but very rarely both.

```{r}
partd_brandchng %>%
  ungroup(.) %>%
  select(`Brand Name`, nomchngspendperclaim, perchngspendperclaim) %>%
  distinct() %>%
  arrange(desc(nomchngspendperclaim)) %>%
  top_n(20, wt = nomchngspendperclaim) %>%
  mutate(Rank = row_number(), perchngspendperclaim = paste0(round(perchngspendperclaim*100, 2), "%")) %>%
  rename("Nominal Average Spend per Claim Delta" = nomchngspendperclaim, "Percentage Average Spend per Claim Delta" = perchngspendperclaim) %>%
  select(Rank, `Brand Name`, `Nominal Average Spend per Claim Delta`, `Percentage Average Spend per Claim Delta`) %>%
  kable(format = "html") %>%
...
partd_brandchng %>%
  ...
  select(`Brand Name`, nomchngspendperclaim, perchngspendperclaim, claims_time) %>%
  distinct() %>%
  ...
  select(Rank, `Brand Name`, `Nominal Average Spend per Claim Delta`, `Percentage Average Spend per Claim Delta`, `Combined Claims 2013-2017`) %>%
  kable(format = "html") %>%
  kable_styling(bootstrap_options = c("striped", "hover"))
```
...
```{r partd_manufacturer, include = FALSE}
...
```{r medicaid_manufacturer, include = FALSE}
#TODO Manufacturer Level
